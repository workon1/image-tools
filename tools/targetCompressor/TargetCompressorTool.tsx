...
import { compressToMaxBytes } from "@/lib/imageCompression";
...
type TargetCompressorToolProps = {
  maxBytes: number;
  toolId: string;
};

export function TargetCompressorTool({ maxBytes, toolId }: TargetCompressorToolProps) {
  const selection = useImageSelection({ toolId });
  ...
  const [outputFormat, setOutputFormat] = useKeyedState<ImageFormat>(imageKey, "jpeg");
  ...
  async function run() {
    ...
    try {
      ...
      logError("target-compress", error);
  ...
  return (
    <section
      aria-label={`Compress to ${formatFileSize(maxBytes)}`}
      aria-busy={working}
    ...
    >
      <div className="space-y-6 p-5 sm:p-7">
        <p className="rounded-2xl bg-accent/10 px-4 py-3 text-sm text-muted">
          We’ll lower quality, then scale the image if needed, until the file is{" "}
          {formatFileSize(maxBytes)} or smaller. JPG and WebP work best for a size cap.
        </p>
          ...
          <>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <button
                ...
                disabled={working}
              >
                {working ? "Compressing..." : `Compress to ${formatFileSize(maxBytes)}`}
              </button>
              <button
                type="button"
                className="btn-ghost"
